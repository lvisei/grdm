...
func (d Download) Do() error {
	d.filePath = fmt.Sprintf("%v/%v", d.TargetDirPath, d.FileName)
...
func (d Download) downloadSection(i int, c [2]int) error {
	...
	err = ioutil.WriteFile(fmt.Sprintf("%v/section-%v.tmp", d.TargetDirPath, i), b, os.ModePerm)
	if err != nil {
		return err
	}
	return nil
...
func (d Download) mergeFiles(sections [][2]int) error {
	...
	defer f.Close()
	for i := range sections {
		tmpFileName := fmt.Sprintf("%v/section-%v.tmp", d.TargetDirPath, i)
